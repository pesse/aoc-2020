...
with
  ...
  regex_pattern as (
    select '([0-9]+)-([0-9]+) ([a-z]): ([a-z]+)' pattern from dual
  ),
  parsed_input as (
    select
...
select
  count(*)
  from parsed_input
  where regexp_count(password, search_char) between min_occurence and max_occurence
;
...
with
  ...
  positions_analyzed as (
    select
      password,
      case
        when substr(password, first_position, 1) = search_char then
          1
        else
          0
        end first_pos_found,
      case
        when substr(password, second_position, 1) = search_char then
          1
        else
          0
